...
export default class Transmission {
  ...
  constructor(data: TransmissionData) {
  ...
  #parseModel() {
    ...
    const _retarder = matches.filter(ent => ent[0].includes('retarder'))[0] ?? {};
    const _subgears = matches.filter(ent => ent[0].includes('subgears'))[0] ?? {};
    const _gears = matches.filter(ent => ent[0].includes('gears'))[0] ?? {};

    const retarder = this.#retarder ? this.#retarder
      : _retarder?.[1] ? true : false;
